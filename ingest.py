...
def ingest(xlsx, poder, ccaa="Euskadi"):
    ...
    for _, r in df.iterrows():
        ...
        conn.execute(
            ...
            (str(r.get("Objeto del contrato", ""))[:300], str(r.get("Código del contrato", "")),
             ...
             str(r.get("CPV", ""))[:10], "datos.gob.es", ccaa))
        n += 1
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    ccaa = sys.argv[3] if len(sys.argv) > 3 else "Euskadi"
    ingest(str(xlsx), poder, ccaa)
